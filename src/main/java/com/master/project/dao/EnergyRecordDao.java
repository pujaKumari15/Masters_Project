...
public interface EnergyRecordDao extends JpaRepository<EnergyRecord, String> {
    ...
    @Query("SELECT e FROM EnergyRecord e WHERE e.userId = :userId AND e.timestamp BETWEEN :startDate AND :endDate")
    List<EnergyRecord> findByUserIdAndDateRange(String userId, Timestamp startDate, Timestamp endDate);
}
